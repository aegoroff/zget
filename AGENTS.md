...
- `--checksum=sha256` / `--checksum=blake3` hashes decompressed output bytes via `std.Io.Writer.hashed` and prints `SHA256:` / `BLAKE3:` hex digests to the summary stream; with `-q`, checksum is not computed or printed.
