...
| `checksum.zig` | Checksum parsing, streaming hash via `std.Io.Writer.hashed`, digest output |
...
- `--checksum=sha256` / `--checksum=blake3` hashes decompressed output bytes in `checksum.Stream` via `std.Io.Writer.hashed` and prints `SHA256:` / `BLAKE3:` hex digests to the summary stream; with `-q`, checksum is not computed or printed.
